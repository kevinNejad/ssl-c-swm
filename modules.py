...
class ContrastiveSWM(nn.Module):
    ...
    def __init__(self, embedding_dim, input_dims, hidden_dim, action_dim,
                 ...
                 ignore_action=False, copy_action=False, ssl_loss='contrastive'):
        super(ContrastiveSWM, self).__init__()
        assert ssl_loss in ['contrastive','vic'],'Invalid loss function'
    ...
    def calc_loss(self, obs, action, next_obs):
        ...
        if self.ssl_loss == 'contrastive':
            return self.contrastive_loss(state, action, next_state)
        else:
            return self.VIC_loss(state, action, next_state)
    ...
    def VIC_loss(self,  state, action, next_state):
        ...
        sim_loss = self.energy(state, action, next_state).mean()

        state = state - state.mean(dim=[0,2], keepdim=True)

        std_state = torch.sqrt(state.var(dim=0) + 1e-4)
        std_loss =  torch.mean(F.relu(1 - std_state), dim=1).sum()
        
        cov_loss = 0
        for s in range(state.size(1)):
            cov_s = (state.T @ state) / state.size(0)
            cov_loss += off_diagonal(cov_s).pow_(2).sum().div(state.size(2))
    ...
    def forward(self, states, action):
        ...
        if not self.ignore_action:
            ...
            if self.copy_action:
                ...
                # print('HEREEEE')
                # print(action.size(),self.action_dim, num_nodes )
                action_vec = utils.to_one_hot(
